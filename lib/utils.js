...
utils.processRewrites = function (rewrites) {
    grunt.log.writeln("grunt-connect-proxy.processRewrites executed");
    var rules = [];
    ...
    Object.keys(rewrites || {}).forEach(function (from) {
        ...
        grunt.log.writeln(" -> processRewrites rule.to: " + rule.to);
...
function onUpgrade(req, socket, head) {
    ...
    proxies.forEach(function(proxy) {
        if (!proxied && req && proxy.config.ws && utils.matchContext(proxy.config.context, req.url)) {
            ...
            var source = req.url;
            var target = (proxy.server.options.secure ? 'wss://' : 'ws://') + proxy.server.options.target.host + (proxy.server.options.target.port ? ':' + proxy.server.options.target.port : '' ) + req.url;
...
utils.proxyRequest = function (req, res, next) {
    var proxied = false;
grunt.log.writeln("grunt-connect-proxy.proxyRequest executed.");
    enableWebsocket(req.connection.server);

    proxies.forEach(function(proxy) {
        grunt.log.writeln("-> proxy.config.context: " + proxy.config.context);
        ...
        if (!proxied && req && utils.matchContext(proxy.config.context, req.url)) {
            ...
            var target = (proxy.server.options.secure ? 'https://' : 'http://') + proxy.server.options.target.host + (proxy.server.options.target.port ? ':' + proxy.server.options.target.port : '' ) + req.url;
